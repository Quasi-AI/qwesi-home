    ...
    <div className="min-h-screen bg-gray-50">
            ...
            <div className="max-w-2xl mx-auto px-4">
              <div className="relative">
                <input
                  ...
                  className="w-full pl-10 sm:pl-12 pr-20 sm:pr-32 py-3 sm:py-4 bg-white/10 backdrop-blur-lg border border-white/20 rounded-xl sm:rounded-2xl text-white placeholder-white/70 focus:outline-none focus:ring-2 focus:ring-white/50 text-base sm:text-lg"
                />
                <Search className="absolute left-3 sm:left-4 top-1/2 transform -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-white/70" />
                <button
                  onClick={handleSearch}
                  disabled={searchLoading}
                  className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-white text-[#5C3AEB] px-3 sm:px-6 py-1.5 sm:py-2 rounded-lg sm:rounded-xl font-semibold hover:bg-gray-100 disabled:opacity-50 transition-colors text-sm sm:text-base"
                >
      ...
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
            <div className="text-center">
              <div className="text-xl sm:text-2xl font-bold text-[#5C3AEB]">{investors.length}+</div>
              <div className="text-xs sm:text-sm text-gray-600">Active Investors</div>
            </div>
            <div className="text-center">
              <div className="text-xl sm:text-2xl font-bold text-[#5C3AEB]">{uniqueCountries.length}+</div>
              <div className="text-xs sm:text-sm text-gray-600">Countries</div>
            </div>
            <div className="text-center">
              <div className="text-xl sm:text-2xl font-bold text-[#5C3AEB]">$50M+</div>
              <div className="text-xs sm:text-sm text-gray-600">Total Capital</div>
            </div>
            <div className="text-center">
              <div className="text-xl sm:text-2xl font-bold text-[#5C3AEB]">95%</div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
              <button
                ...
                className={`flex items-center justify-center gap-2 px-4 py-2 rounded-xl font-semibold transition-colors ${
                  showFilters
        ...
        {error && !loading && (
          <div className="text-center py-8 sm:py-12">
            <div className="bg-red-50 border border-red-200 rounded-2xl p-6 sm:p-8 max-w-md mx-auto">
              <X className="w-10 sm:w-12 h-10 sm:h-12 text-red-500 mx-auto mb-4" />
              <h3 className="text-base sm:text-lg font-bold text-red-700 mb-2">Failed to Load Investors</h3>
        ...
        {!loading && !error && filteredInvestors.length === 0 && (
          <div className="text-center py-8 sm:py-12">
            <Users className="w-20 sm:w-24 h-20 sm:h-24 text-gray-300 mx-auto mb-4" />
            <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-2">No Investors Found</h3>
            ...
            {(searchQuery || Object.values(filters).some(f => f)) && (
              <button
                ...
                className="px-4 sm:px-6 py-2 bg-[#5C3AEB] text-white rounded-lg hover:bg-[#342299] transition-colors text-sm sm:text-base"
              >
                Clear All Filters
              </button>
            )}
        ...
        {!loading && !error && filteredInvestors.length > 0 && (
          <>
            {!isSubscribed && filteredInvestors.length > 3 && (
              <div className="bg-white border-2 rounded-2xl p-4 sm:p-6 mb-4 sm:mb-6" style={{ borderColor: '#5c3aec' }}>
                <div className="text-center">
                  <div className="inline-flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-full mb-3 sm:mb-4" style={{ backgroundColor: '#5c3aec20' }}>
                    <Star className="w-5 h-5 sm:w-6 sm:h-6" style={{ color: '#5c3aec' }} />
                  </div>
                  ...
                  <button
                    ...
                    className="px-4 sm:px-6 py-2 text-white rounded-lg hover:opacity-90 transition-all font-semibold text-sm sm:text-base"
            ...
            <div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
              {paginatedInvestors.map(investor => (
                <div
                  key={investor._id}
                  className="bg-white border border-gray-200 rounded-xl p-4 sm:p-5 hover:shadow-lg transition-all duration-300"
                >
                  ...
                  <div className="flex items-center justify-between pt-3 sm:pt-4 border-t border-gray-100">
                    ...
                    <div className="flex items-center gap-1 sm:gap-2">
                      ...
                      <button
                        ...
                        className="px-2 sm:px-3 py-1 sm:py-1.5 text-white text-xs font-semibold rounded-lg hover:opacity-90 transition-all"
                        style={{ backgroundColor: '#5c3aec' }}
                      >
                        Contact
                      </button>
                    ...
                    {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                      ...
                      return page > 0 && page <= totalPages ? (
                        <button
                          ...
                          className={`px-2 sm:px-3 py-2 text-sm rounded-lg transition-colors whitespace-nowrap ${
                            page === currentPage
                              ? 'text-white'
          ...
          <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-xl">
            <div className="p-4 sm:p-6 border-b border-gray-200">
              <div className="flex items-start justify-between">
                <div className="min-w-0 flex-1">
                  <h2 className="text-xl sm:text-2xl font-bold text-gray-900 truncate">{viewingInvestor.name || 'Unnamed Investor'}</h2>
                  <p className="text-base sm:text-lg font-semibold mt-1 truncate" style={{ color: '#5c3aec' }}>{viewingInvestor.email}</p>
                  {viewingInvestor.companyName && (
                    <p className="text-gray-600 mt-1 truncate text-sm sm:text-base">{viewingInvestor.companyName}</p>
                  )}
                </div>
                <button
                  onClick={closeInvestorModal}
                  className="p-1 sm:p-2 hover:bg-gray-100 rounded-lg transition-colors ml-2 flex-shrink-0"
                >
            ...
            <div className="p-4 sm:p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
              <div className="grid gap-4 sm:gap-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  {viewingInvestor.country && (
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="font-semibold truncate">{viewingInvestor.country}</div>
                      </div>
                    </div>
                  )}
                  {viewingInvestor.phone && (
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="font-semibold truncate">{viewingInvestor.phone}</div>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="font-semibold hover:underline transition-colors truncate block"
                          style={{ color: '#5c3aec' }}
                        >
                          {viewingInvestor.website}
                        </a>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="font-semibold truncate">{formatDate(viewingInvestor.createdAt)}</div>
                    </div>
                  </div>
                </div>
                ...
                {viewingInvestor.investorDetails?.role && viewingInvestor.investorDetails.role.length > 0 && (
                  <div>
                    <h3 className="text-base sm:text-lg font-semibold mb-3">Investor Roles</h3>
                ...
                {viewingInvestor.investorDetails?.pitch && (
                  <div>
                    <h3 className="text-base sm:text-lg font-semibold mb-3">About</h3>
                    <p className="text-gray-700 leading-relaxed text-sm sm:text-base">{viewingInvestor.investorDetails.pitch}</p>
                  </div>
                )}

                {viewingInvestor.investmentDescription && (
                  <div>
                    <h3 className="text-base sm:text-lg font-semibold mb-3">Investment Focus</h3>
                    <p className="text-gray-700 leading-relaxed text-sm sm:text-base">{viewingInvestor.investmentDescription}</p>
                  </div>
                )}
                ...
                {viewingInvestor.investmentDescription2 && (
                  <div>
                    ...
                    <p className="text-gray-700 leading-relaxed text-sm sm:text-base">{viewingInvestor.investmentDescription2}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="p-4 sm:p-6 border-t border-gray-200">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
                <button
                  onClick={closeInvestorModal}
                  className="w-full sm:w-auto px-4 sm:px-6 py-2 text-gray-600 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors text-sm sm:text-base"
                >
                  Close
                </button>
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto">
                  {viewingInvestor.website && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full sm:w-auto px-4 sm:px-6 py-2 text-white rounded-lg hover:opacity-90 transition-colors font-semibold text-center text-sm sm:text-base"
                      style={{ backgroundColor: '#5c3aec' }}
                    >
                      Visit Website
                    </a>
                  ...
                  <button
                    ...
                    className="w-full sm:w-auto px-4 sm:px-6 py-2 text-white rounded-lg hover:opacity-90 transition-colors font-semibold text-sm sm:text-base"
                    style={{ backgroundColor: '#5c3aec' }}
                  >
                    Contact Investor
                  </button>
